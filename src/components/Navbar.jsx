  ...
  return (
    ...
    items-center py-5 fixed top-0 z-20 bg-primary`}>
      ...
      items-center max-w-7xl mx-auto">
        <Link to='/'
        className='fles items-center gap-2'
        onClick={()=>{
          setActive("")
          window.scrollTo(0,0)
        }}>
          <div className='flex items-center'>
            ...
            <p className='text-white text-[18px] font-bold cursor-pointer ml-2'>KADI Amir Khalid</p>
          </div>

        </Link> 
        <ul className='list-none hidden sm:flex flex-row gap-10'>
          {navLinks.map((link) => (
          <li className={`${active === link.title? "text-white"
            : "text-secondary"} hover:text-white text-[10px]
          ...
          justify-end items-center'>
            ...
            <div className={`${!toggle? 'hidden' : 'flex'}
              ...
              flex-col gap-4'>
                {navLinks.map((link) => (
                <li className={`${active === link.title? "text-white"
                  : "text-secondary"} font-poppins font-medium cursor-pointer
                  text-[16px]`
                } onClick={()=> {
                  setToggle(!toggle)
